...
var resizePizzas = function(size) { 
	...
	function changePizzaSizes(size) {
		//Grab all the elements within the randomPizzaContainer Class and place them in an array "pizzas"
		var elemCollection = document.getElementsByClassName("randomPizzaContainer");
		var pizzas = Array.prototype.slice.apply(elemCollection);

		//Grab the current width of randomPizzas
		var windowwidth = document.getElementById("randomPizzas").offsetWidth;

		//Get the previous width of one element in pizzas. Since all the sizes are the same, we only need to grab one.
		var oldwidth = pizzas[0].offsetWidth;

		//Get dx: the new size minus the old size multiplied by the window width
		var dx = determineDx(oldwidth, size, windowwidth);

		//Calculate the new width of each element
		var newwidth = (oldwidth + dx) + 'px';

		//Set the new width for each element in pizzas
...
function updatePositions() {
	...
	//Switched to getElementsByClassName for better performance
	var items = document.getElementsByClassName('mover');

	//Moved scrollTop Variable outside for loop to avoid forced synchronous layout
...
document.addEventListener('DOMContentLoaded', function() {
	var cols = 8;
	var s = 256;
	for (var i = 0; i < 50; i++) { //Reduced number of pizzas from 200 to 50
